...
// Image Path (আপনার পাথ অনুযায়ী ঠিক রাখুন)
...
const Banner = () => {
  return (
    <section className="relative w-full min-h-screen flex flex-col justify-center overflow-hidden bg-white dark:bg-[#020617] transition-colors duration-500 z-10 py-24 lg:py-0">
      ...
      {/* 2. BACKGROUND IMAGE - Fixed position for readability */}
      ...
      <div className="relative z-30 max-w-[1600px] mx-auto w-full px-6 lg:px-4 flex flex-col lg:grid lg:grid-cols-2 gap-12 lg:gap-20 pb-40 lg:pb-0">
        ...
        <div className="flex flex-col justify-center items-center lg:items-end text-center lg:text-right">
           <motion.div 
             ...
             className="max-w-md p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] bg-white/80 dark:bg-white/[0.08] backdrop-blur-xl border border-slate-200 dark:border-white/10 shadow-2xl relative"
           >
              ...
              <a href="#" className="inline-flex items-center gap-3 mt-6 lg:mt-10 text-orange-500 font-black border-b-2 border-orange-500 pb-1 uppercase text-[10px] lg:text-xs tracking-widest hover:text-orange-600 hover:border-orange-600 transition-all">
        ...
        <div className="flex items-center gap-5 lg:gap-8">
            {[<Github key="g" size={20}/>, <Linkedin key="l" size={20}/>, <Twitter key="t" size={20}/>].map((icon, i) => (
                <motion.a
                    ...
                    className="text-slate-400 transition-all p-3 bg-slate-100 dark:bg-white/5 rounded-xl border border-transparent dark:border-white/10"
                >
                    {icon}
                </motion.a>
            ))}
